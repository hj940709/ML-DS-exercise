...
class BiLSTM():
    ...
    def casingInput(self, sentence_length=None):
        ...
        casings = tf.cast(casings, tf.float32)
    ...
    def charInput(self, sentence_length=None):
        ...
        if self.params['character']['charEmbeddings'].lower() == 'lstm':
            ...
            (output_fw, output_bw), _ = \
                tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, chars, dtype=tf.float32, scope = 'char_lstm')
    ...
    def buildModel(self):
        ...
        for i, size in enumerate(self.params['LSTM-Size']):
            lstm_fw_cell = tf.nn.rnn_cell.LSTMCell(size)
            lstm_bw_cell = tf.nn.rnn_cell.LSTMCell(size)
            if isinstance(self.params['dropout'], (list, tuple)):
                ...
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, merged,
                                                            dtype=tf.float32, scope='shared_varLSTM_' + str(i+1))
                ...
                """ Naive dropout """
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, merged,
                                                 dtype=tf.float32, scope='shared_LSTM_' + str(i+1))
                merged = tf.concat([output_fw, output_bw], axis=-1)
                merged = tf.layers.Dropout(self.params['dropout'],
                                                 name='shared_dropout_'+ str(i+1))(merged)
                '''
                ...
                if self.params['dropout'] > 0.0:
                    merged_input = TimeDistributed(Dropout(self.params['dropout']),
                                                   name='shared_dropout_' + str(self.params['dropout']) + "_" + str(
                                                       cnt))(merged_input)
                '''
            print(i+1, 'BiLSTM Shape:', merged.shape)
        ...
        precision, precision_op = tf.metrics.precision(label, output, name='precision')
        recall, recall_op = tf.metrics.recall(label, output, name='recall')
        tf.summary.scalar('loss', lossFct)
        ...
        #optimizerParams['name'] = self.params['optimizer']['type'].lower()
        ...
        gradients, variables = zip(*opt.compute_gradients(lossFct))
        gradients, _ = tf.clip_by_global_norm(gradients, self.params['optimizer']['clipnorm'])
        grad_vars = zip(gradients, variables)
    ...
    def _model_predict(self, dict):
        if self.params['classifier'].lower() == 'softmax':
            ...
            decode_tags, score = self.session.run(['crf_output:0', 'score:0'], feed_dict=dict)
            result = np.array([[[0 if _ != seq else 1 for _ in range(len(self.params['labelEntries']))]
                       for seq in batch]
                      for batch in decode_tags])
        return result

    def fit(self, log_path=None):
        ...
        score = 0
        counter = 0
        directory = log_path or self.params['modelSavePath']
        if not os.path.exists(directory):
            os.makedirs(directory)
        summary_writer = tf.summary.FileWriter(logdir=os.path.join(directory, 'logs'), graph=self.session.graph)
        for epoch in range(self.params['epoch']):
            progress = trange(self.dataset['num_sents'] // self.params['miniBatchSize'] + 1)
            progress.set_description('Epoch '+str(epoch))
            test_batch = next(self.datagenerator)
            for _ in progress:
            ...
            if counter > self.params['earlyStopping']:
                print('Early Stopped')
                break
            summary = self.session.run('Merge/MergeSummary:0', feed_dict=test_batch)
            summary_writer.add_graph(self.session.graph, epoch)
    ...
    def predict(self, tokens, toTag=False):
        ...
        for sent in tokens:
            ...
            y_pred = self._model_predict(x)[0]
            if not toTag:
                sents.append(y_pred)
            else:
    ...
    def evaluate(self, dataset, sortedBySize=False):
        ...
        if sortedBySize:
            ...
            for g in data:
                ...
                y_g_pred = self._model_predict(x)
